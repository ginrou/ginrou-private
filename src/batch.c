...
int batch110827( int argc, char* argv[] )
{
  ...
  freq *psf = (freq*)fftw_malloc(memSize);
  ...
  freq *psf1 = (freq*)fftw_malloc(memSize);
  freq *psf2 = (freq*)fftw_malloc(memSize);
...
int batch110828_1( int argc, char* argv[] ){
  ...
  for( int i = 1; i < kernels; ++i ){
    ...
    psf[i] = (freq*)fftw_malloc(memSize);
...
IMG* errorCount( IMG* input1, IMG* input2, point stPoint, point edPoint )
{
